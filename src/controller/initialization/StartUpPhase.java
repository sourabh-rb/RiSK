...
public class StartUpPhase {
	...
	/**
	 * This function creates objects for all the map elements like country, continent, player
	 * 
	 * @param continentHashMap This hashmap contains continents and their respective values
	 * @param terrritoryHashMap This hashmap contains country as the key element and values comprise continent to which it belongs and the countries neighboring countries
	...
	public void initialSetUp(ArrayList<Player> playerList, ArrayList<Continent> continentList,
		...
		Utilities.gameLog("Total number of continents in the whole map "+continentList.size());
		...
		Utilities.gameLog("Total number of countries in the whole map "+noOfTotalCountries);
		...
		for (int j = 0; j < playerList.size(); j++) {
			...
			Utilities.gameLog("Total number of armies assigned to "+playerList.get(j).getName()+" is "+numberOfArmiesEach);
			for (int k = 0; k < playerList.get(j).getCountries().size(); k++) {
				...
				Utilities.gameLog("No of armies left with "+playerList.get(j).getName()+" is "+playerList.get(j).getNumberOfArmiesLeft());
			}
	...
	public int calculateNoOfArmies(int noOfPlayers) {
		...
		if (noOfPlayers == 2)
			...
			noOfArmies = 20;
		Utilities.gameLog("Assigned " +noOfArmies+" to each player");
		return noOfArmies;
	}
